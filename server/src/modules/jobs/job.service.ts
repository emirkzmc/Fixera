...
export class JobService implements IJobService {
  ...
  async createJob(user: AuthenticatedUser, dto: CreateJobDto): Promise<JobResponseDto> {
    ...
    const result = await this.databaseService.query(
      `INSERT INTO jobs (workshop_id, customer_name, item_identifier, issue_description, tracking_code) 
       VALUES ($1, $2, $3, $4, $5) 
       RETURNING id, workshop_id, customer_name, item_identifier, issue_description, status, tracking_code, created_at, updated_at`,
      [tenantId, dto.customerName, dto.itemIdentifier, dto.issueDescription || null, trackingCode],
    );
  ...
  async getJobsByWorkshop(user: AuthenticatedUser): Promise<JobResponseDto[]> {
    const tenantId = this.resolveTenantId(user);
    const result = await this.databaseService.query(
      `SELECT id, workshop_id, customer_name, item_identifier, issue_description, status, tracking_code, created_at, updated_at 
       FROM jobs 
       WHERE workshop_id = $1 
  ...
  async updateJob(user: AuthenticatedUser, jobId: string, dto: UpdateJobDto): Promise<JobResponseDto> {
    ...
    const query = `UPDATE jobs SET ${updates.join(', ')} WHERE id = $${paramIndex++} AND workshop_id = $${paramIndex} RETURNING id, workshop_id, customer_name, item_identifier, issue_description, status, tracking_code, created_at, updated_at`;
    values.push(jobId, tenantId);

    const result = await this.databaseService.query(query, values);
    const updatedJob = this.mapToDto(result.rows[0]);

    // O SİHİRLİ AN: Kullanıcıya anlık bildirim fırlatıyoruz (Eğer statü falan değiştiyse)
